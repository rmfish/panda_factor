<template>
  <section class="view">
    <header>
      <h2>Data Hub Data Clean</h2>
      <p>股票行情清洗：/datahub/api/v1/upsert_stockmarket_final</p>
    </header>
...
const startClean = async () => {
  ...
  message.value = '';
  try {
    const response = await apiGet('/datahub/api/v1/upsert_stockmarket_final', {
...
const loadProgress = async () => {
  try {
    const response = await apiGet('/datahub/api/v1/get_progress_stock_final');
